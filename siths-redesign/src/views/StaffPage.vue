<template>
    <div class="h-fit mx-10">
      ...
      <SearchBar class="w-2/3" type="text" v-model="searchInput" placeholder="Search by name or email" />
...
function sortbySearch(searchInput){
    if (searchInput !== " ") {
        return staff.value.filter((staffMember) => staffMember.name.toLowerCase().includes(searchInput.toLowerCase()))
        || staffMember.email.toLowerCase().includes(searchInput.toLowerCase());
    }
}
